...
if nfolds > 1:
    for (inTr, inTe) in folds:
        ...
        for j in range(nbags):
            ...
            model.fit(xtr, ytr, nb_epoch=50, batch_size=1000, validation_data=[xte, yte])
            ...
            pred += model.predict_proba(x=xte)

            pred_test += model.predict_proba(x=testset)
    ...
    for j in range(nbags):
        print(j)
        model = nn_model()
        model.fit(train_X, train_y, nb_epoch=1200, batch_size=1000)
        pred_test += model.predict_proba(x=testset)
        print(str(datetime.timedelta(seconds=time() - begintime)))
